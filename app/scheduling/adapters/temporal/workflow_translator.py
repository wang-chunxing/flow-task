...
class WorkflowTranslator:
    ...
    def translate(self, task: Task) -> Any:  # 返回动态生成的工作流类
        # 处理依赖关系
        ...
        # 生成工作流名称
        workflow_name = f"DynamicWorkflow_{task.name}_{uuid.uuid4().hex}"

        # 定义工作流方法
        async def run(self, input_data: Any) -> Any:
            execution_ctx = WorkflowExecutionContext()
            return await self._execute_layer(
                task.workflow.execution_layers, input_data, execution_ctx
            )

        async def _execute_layer(self, layers: List[List[Operator]], initial_input: Any, ctx: WorkflowExecutionContext) -> Any:
        ...
        async def _execute_operator(self, operator: Operator, input_data: Any, ctx: WorkflowExecutionContext) -> Any:
            for dep in self.__temporal_activities_dependencies__[operator.name]:
                if ctx.get_result(dep) is None:
                    raise ApplicationError(f"Unsatisfied dependency: {dep}")

            activity_name = f"op_{operator.name}"
            act_def = next(a for a in self.__temporal_activities__
        ...
        # 动态创建类
        DynamicWorkflow = type(workflow_name, (object,), class_attrs)
        # 应用装饰器
        DynamicWorkflow = workflow.defn(DynamicWorkflow)
