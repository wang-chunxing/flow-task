...
class WorkflowTranslator:
    ...
    def translate(self, task: Task) -> workflow.defn:
        ...
        activities = [self._create_activity(op) for layer in task.workflow.execution_layers for op in layer]

        # 动态构建类方法
        async def run_method(self, input_data: Any) -> Any:
            return await self._execute_workflow(task.workflow.execution_layers, input_data)

        # 定义类方法
        async def _execute_workflow(self, layers: List[List[Operator]], input_data: Any) -> Any:
            execution_ctx = WorkflowExecutionContext()
            current_data = input_data
            for layer in layers:
                results = await asyncio.gather(*[
                    self._execute_operator(operator, current_data, execution_ctx)
        ...
        async def _execute_operator(self, operator: Operator, input_data: Any, ctx: WorkflowExecutionContext) -> Any:
            for dep in self.__temporal_activities_dependencies__.get(operator.name, []):
                if ctx.get_result(dep) is None:
                    raise ApplicationError(f"Unsatisfied dependency: {dep} for operator {operator.name}")
        ...
        def _merge_results(results: List[Any]) -> Any:
            ...
            try:
                ...
                raise ApplicationError(f"Result merge failed: {str(e)}") from e
        ...
        DynamicWorkflow = type(
            ...
            (workflow.defn,),
            {
                ...
                "run": workflow.run(run_method),
                "_execute_workflow": _execute_workflow,
