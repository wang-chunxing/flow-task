...
class WorkflowTranslator:
    ...
    def translate(self, task: Task) -> Type:
        """动态生成符合 Temporal 要求的工作流类"""
        # 生成唯一类名
        ...
        class_template = dedent(f'''
            ...
            class {class_name}:
                _dependencies = {self._process_dependencies(task)}
                _activities = {self._create_activities(task)}

                @workflow.run
                async def run(self, input_data):
                    ctx = WorkflowExecutionContext()
                    return await self._execute_layers(
                        {task.workflow.execution_layers}, 
                        input_data, 
                        ctx
                    )

                async def _execute_layers(self, layers, input_data, ctx):
                ...
                async def _execute_operator(self, operator, input_data, ctx):
                    # 依赖检查
                    ...
                    activity = next(
                        ...
                        if a.__temporal_activity_definition__["name"] == activity_name
                    )
